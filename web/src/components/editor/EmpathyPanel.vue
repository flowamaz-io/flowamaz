    ...
    <div v-else-if="analysis" class="flex-1 overflow-y-auto p-4 space-y-4">
      ...
      <div class="grid grid-cols-2 gap-2">
        <div class="bg-gray-50 rounded-lg px-3 py-2 border border-gray-200">
          ...
          <div class="text-lg font-semibold text-gray-900">{{ analysis.emailsSentToRequester }}</div>
        </div>
        <div class="bg-gray-50 rounded-lg px-3 py-2 border border-gray-200">
          <div class="text-xs text-gray-500">Wait periods</div>
          <div class="text-lg font-semibold text-gray-900">{{ analysis.waitPeriodCount }}</div>
        </div>
        <div class="bg-gray-50 rounded-lg px-3 py-2 border border-gray-200">
          <div class="text-xs text-gray-500">Status updates</div>
          <div class="text-lg font-semibold text-gray-900">{{ analysis.statusUpdateCount }}</div>
        </div>
        <div class="bg-gray-50 rounded-lg px-3 py-2 border border-gray-200">
          <div class="text-xs text-gray-500">Avg days to outcome</div>
          <div class="text-lg font-semibold text-gray-900">{{ analysis.avgDaysToOutcome }}</div>
        </div>
      </div>
      ...
      >
        <div class="text-emerald-700 text-sm font-medium">Great workflow experience</div>
      ...
      <div v-else class="space-y-2">
        <div class="text-xs text-gray-500 uppercase tracking-wider font-medium">Issues ({{ analysis.issues.length }})</div>
        <div
        ...
        >
          ...
          >
            ...
            <div class="flex-1 min-w-0">
              <div class="text-sm font-medium text-gray-900 truncate">{{ issue.label }}</div>
              <div class="text-xs text-gray-500 truncate">{{ issueTypeLabel(issue.type) }}</div>
            </div>
            <span class="text-gray-400 text-xs shrink-0">{{ expandedIssues.has(index) ? '▲' : '▼' }}</span>
          </button>

          <div v-if="expandedIssues.has(index)" class="px-4 pb-4 space-y-3 border-t border-gray-200">
            <p class="text-xs text-gray-600 leading-relaxed pt-3">{{ issue.description }}</p>
            <div class="bg-violet-50 border border-violet-200 rounded-lg px-3 py-2">
              <div class="text-xs text-violet-700 font-medium mb-1">How to fix</div>
              <p class="text-xs text-violet-800 leading-relaxed">{{ issue.suggestion }}</p>
            </div>
          </div>
        </div>
      </div>

  ...
  <Transition name="slide-up">
    ...
    >
      ...
      <div class="flex items-center justify-between px-5 py-3 border-b border-gray-200 bg-gray-50">
        ...
        <button class="text-gray-400 hover:text-gray-700 text-sm" @click="$emit('close')">✕</button>
      </div>
      ...
      <div class="flex flex-col h-[calc(100%-3rem)] p-4 gap-3 overflow-hidden">
        ...
        <div class="relative">
          ...
          <!-- Suggestions -->
          <div v-if="suggestions.length && command" class="absolute top-full left-0 right-0 mt-1 bg-white border border-gray-200 rounded-xl overflow-hidden z-10 shadow-md">
            <button
        ...
        <div class="flex-1 overflow-y-auto space-y-3">
          <div v-if="loading" class="text-sm text-teal-600 animate-pulse">Thinking…</div>

          <div v-if="lastResult?.error" class="text-sm text-red-700 bg-red-50 border border-red-200 rounded-xl px-4 py-3">
            {{ lastResult.error }}
          </div>

          <div v-if="lastResult?.pattern" class="text-sm text-emerald-700 bg-emerald-50 border border-emerald-200 rounded-xl px-4 py-2">
            ✓ Applied pattern: <strong>{{ lastResult.pattern }}</strong>
          </div>

          <div v-if="showDiff && lastResult?.patch" class="space-y-2">
            <p class="text-xs text-gray-500 uppercase tracking-wide">What changed</p>
          ...
          <div v-if="history.length && !lastResult" class="space-y-1">
            <p class="text-xs text-gray-400 uppercase tracking-wide">Recent commands</p>
            <button
              v-for="h in history"
              :key="h"
